...
const todoCount = (annotatedSchema.match(/# TODO\(barwise\):/g) ?? []).length;
const noteCount = (annotatedSchema.match(/# NOTE\(barwise\):/g) ?? []).length;

writeFileSync(schemaPath, annotatedSchema);
console.log(`Annotated ${schemaPath}`);
console.log(`  ${todoCount} TODO(barwise) comments injected`);
console.log(`  ${noteCount} NOTE(barwise) comments injected`);
console.log();
