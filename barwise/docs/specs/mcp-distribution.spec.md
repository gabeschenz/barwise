...
Two distribution modes for the barwise MCP server:
...
2. **External tool users** (Claude Code, Cursor, Windsurf, Claude
   Desktop, etc.): Run `npx @barwise/mcp` -- no clone, no build.
...
1. **esbuild.mjs** -- Add a third entry point that bundles the MCP
   server code (from `@barwise/mcp`) into `dist/mcp/index.js`. This
   bundles core, diagram, llm, MCP SDK, and zod into a single file.

2. **package.json** -- Add:
   - `contributes.mcpServerDefinitionProviders` with id `barwise.mcpServer`
   - `barwise.enableMcpServer` boolean setting (default: `true`)
...
3. **src/mcp/McpServerProvider.ts** -- New module that implements
   ...
   - `provideMcpServerDefinitions()` checks the `barwise.enableMcpServer`
...
5. **src/mcp/stdio-entry.ts** -- Standalone entry point that imports
   `createServer` from `@barwise/mcp/server` and connects stdio
...
- To disable: set `barwise.enableMcpServer` to `false`.
- No config files, no build steps, no absolute paths.

### Part B: Standalone npx Binary

For users outside VS Code, `@barwise/mcp` needs to be publishable to
...
2. **packages/mcp/package.json** -- Changes for npm publishing:
   ...
   - Move workspace dependencies (`@barwise/core`, `@barwise/diagram`,
     `@barwise/llm`) to devDependencies (they are bundled, not needed
     at install time)
...
1. **VS Code users** -- "Install the Barwise ORM Modeler extension.
   The MCP server is enabled by default. You can toggle it via the
   `barwise.enableMcpServer` setting."

2. **Other AI tools** -- Show `npx @barwise/mcp` config for each
