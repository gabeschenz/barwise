...
`barwise import transcript` for each transcript/model combination.
With 3 transcripts and 2 models, that is 6 manual invocations with
the same boilerplate options each time.

## Solution

Add `barwise import batch <dir>` subcommand that runs all `.md`
...
```bash
barwise import batch examples/transcripts/ \
