# BARWISE-5ln: Annotate .orm.yaml from transcript import with actionable TODOs
...
Inject `# TODO(barwise):` and `# NOTE(barwise):` comments directly into the
...
The annotator lives in `@barwise/core` (not `@barwise/llm`) to avoid creating
a dependency from core to llm. Instead, a structural `TranscriptProvenance`
interface mirrors the shape of `DraftModelResult` from `@barwise/llm`. The
VS Code command passes `DraftModelResult` directly, which satisfies the
interface structurally.

### Shared annotation helpers

`stripBarwiseComments()`, `formatBarwiseComment()`, and `truncate()` were
...
`stripBarwiseComments()` removes all existing barwise annotations before
