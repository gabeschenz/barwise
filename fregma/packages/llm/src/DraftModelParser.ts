...
import type { ConceptualDataTypeName, DataTypeDef, RingType } from "@fregma/core";
import type {
  ExtractionResponse,
  DraftModelResult,
  ElementProvenance,
...
export function parseDraftModel(
  ...
  for (const ic of response.inferred_constraints) {
    ...
    if (ic.type === "internal_uniqueness") {
      ...
      if (roleIds.length === 0) {
        ...
        const constraint: import("@fregma/core").Constraint = {
          type: ic.type,
          roleIds,
        };
        if (isDuplicateConstraint(ft, constraint)) {
      ...
      if (roleIds.length !== 1) {
        ...
        const freqConstraint: import("@fregma/core").Constraint = {
          type: "frequency",
          roleId: roleIds[0]!,
          min: ic.min,
          max: ic.max,
      ...
      if (roleIds.length !== 2) {
        ...
        const ringConstraint: import("@fregma/core").Constraint = {
          type: "ring",
          roleId1: roleIds[0]!,
          roleId2: roleIds[1]!,
      ...
      if (subsetRoleIds.length === 0 || supersetRoleIds.length === 0) {
        ...
        const constraint: import("@fregma/core").Constraint = ic.type === "subset"
  ...
  // Pass 5: Create objectified fact types.
