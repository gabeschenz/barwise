...
This package depends on `@fregma/core` (model types, serializers) and
...
`@fregma/diagram` is a devDependency only (used in integration tests).
...
- The extraction prompt uses the JSON Schema from `@fregma/core` to
...
|-----------|---------|--------------|
| Upstream  | `@fregma/core` | `OrmModel`, `ObjectType`, `FactType`, `Role`, constraint types, serialization |
| Downstream | `fregma-vscode` | `processTranscript`, `AnthropicLlmClient`, extraction types |
