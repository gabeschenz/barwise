...
// Mock the @fregma/llm module
vi.mock("@fregma/llm", async () => {
  const actual = await vi.importActual("@fregma/llm");
  return {
    ...actual,
    reviewModel: vi.fn(),
...
describe("review_model tool", () => {
  ...
  it("calls reviewModel with the resolved model", async () => {
    const { reviewModel } = await import("@fregma/llm");
  ...
  it("passes provider and model options to createLlmClient", async () => {
    const { reviewModel, createLlmClient } = await import("@fregma/llm");
