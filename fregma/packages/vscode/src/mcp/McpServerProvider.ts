 ...
 *
 * When `fregma.enableMcpServer` is true, returns an McpStdioServerDefinition
 ...
 *
 * The provider watches the `fregma.enableMcpServer` setting and fires
...
export function registerMcpServerProvider(
      ...
      provideMcpServerDefinitions(
        ...
        if (!fs.existsSync(mcpModule)) {
          void vscode.window.showErrorMessage(
            "Fregma: bundled MCP server not found at " +
              mcpModule +
  ...
  const settingWatcher = vscode.workspace.onDidChangeConfiguration((e) => {
    if (e.affectsConfiguration("fregma.enableMcpServer")) {
      onDidChange.fire();
    }
  });
