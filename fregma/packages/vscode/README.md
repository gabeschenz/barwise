...
|---------|---------|-------------|
| `fregma.llmProvider` | `copilot` | LLM provider: `copilot` (free with GitHub Copilot) or `anthropic` |
| `fregma.anthropicApiKey` | | Anthropic API key (only when provider is `anthropic`) |
| `fregma.anthropicModel` | `claude-sonnet-4-5-20250929` | Anthropic model ID |
| `fregma.copilotModelFamily` | | Preferred Copilot model family |
