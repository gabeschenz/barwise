 ...
 *
 * Comment prefixes:
 *   - `# TODO(fregma):` for gaps and warnings that need human action
 *   - `# NOTE(fregma):` for informational notes (e.g. source-resolved types)
 *
...
export function annotateDbtYaml(
  ...
  if (entries.length === 0) return stripFregmaComments(yamlContent);

  // Strip any existing fregma annotations to ensure idempotency.
