 ...
 *
 * The mapper resolves NORMA XML id references into Fregma model
 * objects and translates NORMA constraint representations into
 * the Fregma Constraint discriminated union.
 *
...
export function mapNormaToOrm(doc: NormaDocument): OrmModel {
  ...
  for (const nft of doc.factTypes) {
    ...
    // Record role id mappings (NORMA id -> Fregma id).
    // Since we pass NORMA role ids as the id in RoleConfig, the
    // Fregma role ids will be the same NORMA ids (FactType constructor
 ...
 * NORMA uses "{0}", "{1}" placeholders which match Fregma's format.
 */
...
function generateFactTypeName(
  ...
  const playerNames = nft.roles.map((r) => {
    const fregmaId = objectTypeIdMap.get(r.playerRef);
    if (fregmaId) {
      const ot = model.getObjectType(fregmaId);
...
/**
 * Map a single NORMA constraint to a Fregma Constraint.
  ...
  for (const nc of doc.constraints) {
    ...
    for (const roleRef of nc.roleRefs) {
      for (const nft of doc.factTypes) {
        ...
        // Find the corresponding Fregma fact type.
