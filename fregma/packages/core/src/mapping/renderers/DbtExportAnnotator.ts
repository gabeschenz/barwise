 ...
 *
 *   - `# TODO(fregma):` for actionable gaps (missing descriptions,
 *     default data types needing review)
 *   - `# NOTE(fregma):` for informational notes (composite PKs,
...
export interface ExportAnnotation {
  ...
  /** Severity: "todo" produces `# TODO(fregma):`, "note" produces `# NOTE(fregma):`. */
...
export function annotateDbtExport(
  ...
  if (annotations.length === 0) {
    return { schemaYaml: stripFregmaComments(schemaYaml), annotations };
  }

  const cleanYaml = stripFregmaComments(schemaYaml);
