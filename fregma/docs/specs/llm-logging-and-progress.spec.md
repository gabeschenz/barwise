...
Add a `fregma.verboseLogging` boolean setting that, when enabled,
...
in `@fregma/llm`. This is backward-compatible since the fields are
optional.
...
Add the `fregma.verboseLogging` setting and write LLM interaction
